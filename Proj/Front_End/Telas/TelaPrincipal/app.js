...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('anoFooter') && (document.getElementById('anoFooter').textContent = new Date().getFullYear());
    renderDestaque();
    renderProdutos();
    atualizarBadge();
});
...
const _a = ["Ad","min"].join("");
const _b = ["Ad","minAnik"].join("");
...
/* ── Sistema de filtros da sidebar ───────────────────────────── */

/**
 * Coleta todos os filtros activos da sidebar e devolve
 * os produtos que satisfazem qualquer um deles.
 * Lógica: OR entre categorias; dentro de cada categoria,
 * AND entre tipos de filtro (marca AND preço AND stock).
 */
function aplicarFiltros() {
    // Se não há produtos, mostra estado vazio directamente
    if (produtos.length === 0) {
        renderProdutosLista([]);
        return;
    }

    // Recolhe filtros globais
    ...
    // Categorias configuradas na sidebar
    const cats = ['celulares','laptop','monitor','fone','teclado','mouse','gpu','relogio','ram','cooler','cpu','placamae'];

    // Para cada produto, verifica se passa nos filtros
    const lista = produtos.filter(p => {
        // ─ Filtros globais (aplicam-se sempre) ─
        ...
        // Apenas aplica filtros de categoria se algum estiver activo nessa categoria
        for (const cat of cats) {
            ...
            const temFiltroActivo = marcas.length || precoMin !== null || precoMax !== null || stock;
            if (!temFiltroActivo) continue;  // nenhum filtro activo nesta cat → ignora

            // O produto deve pertencer a esta categoria para ser afectado
...
function marcasSelecionadas(cat) {
    return [...document.querySelectorAll(`.filtro-marca[data-cat="${cat}"]:checked`)]
        .map(el => el.value);
}
...
/* ── Compat. com links do footer ─────────────────────────────── */
function filtrar(cat) {
    // Mapeia categorias antigas → grupo da sidebar
    const mapa = {
        todos:      null,
