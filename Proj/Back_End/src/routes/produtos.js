...
// Cache desabilitado para evitar inconsistências
// const cache = new Map();
...
// ==================== FUNÇÃO AUXILIAR: converter preço de centavos para euros ====================
...
router.get('/destaque/lista', (req, res) => {
  try {
    ...
    const response = { success: true, produtos: produtosConvertidos, total: produtosConvertidos.length };
    return res.json(response);
  } catch (err) {
    console.error('Erro ao buscar produtos em destaque:', err);
    return res.status(500).json({ erro: 'Erro ao buscar produtos em destaque.' });
  }
});
...
router.get('/', (req, res) => {
  try {
    ...
    // 🔧 min e max em centavos (vêm em unidades, converter)
    if (min !== undefined && min > 0) {
      query += ' AND p.preco >= ?';
      params.push(Number(min) * 100);
    }

    if (max !== undefined && max > 0) {
      query += ' AND p.preco <= ?';
      params.push(Number(max) * 100);
    }
    ...
    if (q) {
      countQuery += ` AND (LOWER(p.nome) LIKE LOWER(?) OR LOWER(p.descricao) LIKE LOWER(?) OR LOWER(p.marca) LIKE LOWER(?))`;
      countParams.push(`%${q}%`, `%${q}%`, `%${q}%`);
    }

    const total = db.prepare(countQuery).get(...countParams)?.total || 0;

    // ✅ Converter preços de centavos para unidade
    const produtosConvertidos = converterPrecosArray(result);

    // ✅ Retornar array diretamente para compatibilidade com frontend
    return res.json(produtosConvertidos);
  } catch (err) {
...
router.get('/:id', validarId, (req, res) => {
  try {
    ...
    const relacionados = db.prepare(`
      SELECT id, nome, preco, imagem, em_destaque
      FROM produtos
      WHERE categoria_id = ? AND id != ?
      LIMIT 4
    `).all(produto.categoria_id, id);

    // ✅ Converter preços
    const produtoConvertido = converterPreco(produto);
    const relacionadosConvertidos = converterPrecosArray(relacionados);

    return res.json({ success: true, produto: produtoConvertido, relacionados: relacionadosConvertidos });
  } catch (err) {
...
router.patch('/:id/stock', autenticar, somenteAdmin, validarId, (req, res) => {
  try {
    ...
    if (stock === undefined || stock < 0) {
      return res.status(400).json({ erro: 'Stock inválido. Deve ser um número não negativo.' });
    }
    ...
    res.json({
      success: true,
      mensagem: 'Stock atualizado com sucesso.',
      produto_id: id,
      novo_stock: stock
    });
  } catch (err) {
